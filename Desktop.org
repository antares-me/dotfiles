...
Этот файл содержит конфигурацию для моей среды рабочего стола.
Мой рабочий процесс управляется Emacs, но все еще есть ряд приложений,
инструментов, тем и шрифтов, которые я должен установить, чтобы иметь
...
Загружаем системные настройки из =.emacs.d/per-system-settings.el= и
распаковываем соответствующие значения в блоки, чтобы их можно было использовать
в блоках файла конфигурации. Эти параметры настраиваются в [[file:Systems.org::*Per-System Settings][Systems.org]].
...
#+begin_src emacs-lisp :var name="nil" :session system-settings
  (antares/system-settings-get (intern name))
#+end_src

* Fonts and Themes

Я использую [[https://github.com/derat/xsettingsd][xsettingsd]] в качестве минимального демона настроек приложений
Xorg. Он заменяет аналогичные демоны из окружений рабочего стола, таких как
GNOME и XFCE и позволяет мне использовать простой файл конфигурации, например
следующий:

*.config/xsettingsd/xsettingsd.conf:*

#+begin_src conf :tangle fonts/.config/xsettingsd/xsettingsd.conf :noweb yes
...
Мне также нужно сделать дополнительный шаг, чтобы убедиться, что Emacs может
найти путь к шрифту в профиле «desktop». 

*.config/fontconfig/fonts.conf:*

#+begin_src xml :tangle fonts/.config/fontconfig/fonts.conf
...
(* 1024 (alist-get 'desktop/dpi antares/system-settings))
#+end_src
...
Я использую Emacs в качестве менеджера рабочих столов благодаря отличному EXWM.

#+begin_src emacs-lisp :tangle emacs/.emacs.d/lisp/antares-desktop.el
...
#+begin_src emacs-lisp :tangle emacs/.emacs.d/lisp/antares-desktop.el
  ...
  (defun antares/exwm-init-hook ()
    ...
    (exwm/run-in-background "redshift -l 51.572253:45.996457 & -t 6500:2700 -b 0.80:0.85"))
  ...
  (use-package exwm
    ...
    (defun antares/setup-window-by-class ()
      (interactive)
      (pcase exwm-class-name
        ...
        ("mpv" (exwm-floating-toggle-floating)
               (antares/exwm-floating-toggle-pinned))
        ("gsi" (exwm-input-toggle-keyboard))))

    ;; Do some post-init setup
    (add-hook 'exwm-init-hook #'antares/exwm-init-hook)

    ;; Manipulate windows as they're created
    (add-hook 'exwm-manage-finish-hook
              (lambda ()
                ;; Send the window where it belongs
                (antares/setup-window-by-class)))
...
(defun antares/run-xmodmap ()
  (interactive)
  (start-process-shell-command "xmodmap" nil "xmodmap ~/.dotfiles/.config/i3/Xmodmap"))

(defun antares/update-wallpapers ()
  (interactive)
  (start-process-shell-command
   "feh" nil
   (format "feh --bg-scale ~/.dotfiles/backgrounds/%s" (alist-get 'desktop/background antares/system-settings))))

(setq antares/panel-process nil)
(defun antares/kill-panel ()
  (interactive)
  (when antares/panel-process
    (ignore-errors
      (kill-process antares/panel-process)))
  (setq antares/panel-process nil))

(defun antares/start-panel ()
  (interactive)
  (antares/kill-panel)
  (setq antares/panel-process (start-process-shell-command "polybar" nil "polybar panel")))

(defun antares/update-screen-layout ()
  (interactive)
...
(defun antares/configure-desktop ()
  (interactive)
    (antares/run-xmodmap)
    (antares/update-screen-layout)
    (run-at-time "2 sec" nil (lambda () (antares/update-wallpapers))))

(defun antares/on-exwm-init ()
  (antares/configure-desktop)
  (antares/start-panel))

(when antares/exwm-enabled
  ;; Configure the desktop for first load
  (add-hook 'exwm-init-hook #'antares/on-exwm-init))
...
(defun antares/send-polybar-hook (name number)
  (start-process-shell-command "polybar-msg" nil (format "polybar-msg hook %s %s" name number)))

(defun antares/update-polybar-exwm (&optional path)
  (antares/send-polybar-hook "exwm" 1)
  (antares/send-polybar-hook "exwm-path" 1))

(defun antares/update-polybar-telegram ()
  (antares/send-polybar-hook "telegram" 1))

(defun antares/polybar-exwm-workspace ()
...
(defun antares/polybar-exwm-workspace-path ()
  (let ((workspace-path (frame-parameter nil 'bufler-workspace-path-formatted)))
    (if workspace-path
        (substring-no-properties workspace-path)
      "")))

(defun antares/polybar-mail-count (max-count)
  (if (and antares/mail-enabled antares/mu4e-inbox-query)
    (let* ((mail-count (shell-command-to-string
                         (format "mu find --nocolor -n %s \"%s\" | wc -l" max-count antares/mu4e-inbox-query))))
      (format " %s" (string-trim mail-count)))
    ""))

(defun antares/telega-normalize-name (chat-name)
  (let* ((trimmed-name (string-trim-left (string-trim-right chat-name "}") "◀{"))
         (first-name (nth 0 (split-string trimmed-name " "))))
    first-name))

(defun antares/propertized-to-polybar (buffer-name)
  (if-let* ((text (substring-no-properties buffer-name))
    ...
    (format "%%{F%s}%s%%{F-}" fg-color (antares/telega-normalize-name text))
    text))

(defun antares/polybar-telegram-chats ()
  (if (> (length tracking-buffers) 0)
    (format " %s" (string-join (mapcar 'antares/propertized-to-polybar tracking-buffers) ", "))
    ""))

(add-hook 'exwm-workspace-switch-hook #'antares/update-polybar-exwm)
(add-hook 'bufler-workspace-set-hook #'antares/update-polybar-exwm)
...
#+begin_src conf :tangle polybar/.config/polybar/config :noweb yes
...
[module/exwm]
type = custom/ipc
hook-0 = emacsclient -e "(antares/polybar-exwm-workspace)" | sed -e 's/^"//' -e 's/"$//'
initial = 1
...
[module/exwm-path]
type = custom/ipc
hook-0 = emacsclient -e "(antares/polybar-exwm-workspace-path)" | sed -e 's/^"//' -e 's/"$//'
...
[module/mu4e]
type = custom/ipc
hook-0 = emacsclient -e '(antares/polybar-mail-count 500)' | sed -e 's/^"//' -e 's/"$//'
initial = 1
format-underline = ${colors.underline-1}
click-left = emacsclient -e '(antares/go-to-inbox)'

[module/telegram]
type = custom/ipc
hook-0 = emacsclient -e '(antares/polybar-telegram-chats)' | sed -e 's/^"//' -e 's/"$//'
format-padding = 3
initial = 1
...
#+begin_src sh :tangle polybar/.config/polybar/player-status.sh :shebang #!/bin/sh
...
#+begin_src conf :tangle dunst/.config/dunst/dunstrc :noweb yes
...
[global]
    ...
    icon_path = /home/antares/.guix-extra-profiles/desktop/desktop/share/icons/gnome/256x256/status/:/home/antares/.guix-extra-profiles/desktop/desktop/share/icons/gnome/256x256/devices/:/home/antares/.guix-extra-profiles/desktop/desktop/share/icons/gnome/256x256/emblems/
...
#+begin_src sh :tangle bin/.bin/update-screens :shebang #!/bin/sh
...
#+begin_src conf :tangle types/.config/mimeapps.list
...
#+begin_src scheme :tangle cron/.config/cron/pass-sync.guile
...
#+begin_src python :tangle qutebrowser/.config/qutebrowser/config.py :noweb yes
...
#+begin_src conf :tangle qutebrowser/.config/qutebrowser/quickmarks
...
dot https://github.com/antares-me/dotfiles
...
#+begin_src conf :tangle vimb/.config/vimb/config :noweb yes

# Set the home page to a local file
set home-page=file:///home/antares/.config/vimb/home.html
...
#+begin_src sh :tangle bin/.bin/sync-passwords :shebang #!/bin/sh
...
#+begin_src conf :tangle mpv/.config/mpv/mpv.conf :noweb yes
...
#+begin_src scheme :tangle games/.config/guix/manifests/games.scm :noweb yes
...
#+begin_src conf :tangle zathura/.config/zathura/zathurarc
...
#+begin_src scheme :tangle guix/.config/guix/manifests/desktop.scm :noweb yes
...
#+begin_src shell
  ...
  system-alias "antares-bt" # To configure your laptop's device name
  default-agent
  power on
  scan on
...
* Provide the antares-desktop package

#+begin_src emacs-lisp :tangle emacs/.emacs.d/lisp/antares-desktop.el

  (provide 'antares-desktop)
